// games/alphabet-listen/alphabet-listen.js
// GAME: Nghe Chọn Chữ - bé nghe âm chữ rồi chọn đúng chữ.
...
function __eduRandomItem(list) {
    ...
    registerGame('alphabet_listen', {
        questionTimeSec: 12,

        generateData() {
            return __eduRandomItem(LETTERS);
        },

        renderDisplay(data) {
            return `
                <div class="alphabet-listen-question" onclick="playQuestionAudio()">
                    <div class="alphabet-listen-icon">👂</div>
                    <div class="alphabet-listen-text">Nghe và chọn chữ đúng</div>
                    <div class="alphabet-listen-small">Bấm vào khung để nghe lại</div>
                </div>
            `;
        },
        ...
        getOptions(correct) {
            ...
            while (set.size < 4) {
                const item = __eduRandomItem(LETTERS);
        ...
        styleOptionBtn(btn, item) {
            btn.classList.add('alphabet-listen-option');
            btn.innerHTML = `
                <span class="alphabet-listen-upper">${item.id.toUpperCase()}</span>
                <span class="alphabet-listen-lower">${item.id}</span>
            `;
            btn.setAttribute('aria-label', 'Chữ ' + item.id);
        },
