// js/content.js
// NƠI DUY NHẤT ĐỂ QUẢN LÝ BANNER VÀ BÀI VIẾT

// --- DỮ LIỆU CHO BANNER ---
const bannerSlides = [
    {
        imageUrl: "img/banners/banner1.jpg",
        link: "/link-den-bai-viet-su-kien-1.html" // Ví dụ: "/bai-viet/giam-gia-thang-9"
    },
    {
        imageUrl: "img/banners/banner2.jpg",
        link: "/link-den-san-pham-2.html"
    },
    {
        imageUrl: "img/banners/banner3.jpg",
        link: "https://youtube.com" // Link có thể dẫn ra ngoài trang
    }
];


// --- DỮ LIỆU CHO BÀI VIẾT MỚI NHẤT ---
const latestPosts = [
    {
        title: "Hướng dẫn đăng ký tài khoản ngân hàng Seven Bank",
        summary: "ngoài yucho thì đây là ngân hàng thứ 2 cho phép mọi người mới đến Nhật Bản có thể đăng ký dùng...",
        imageUrl: "img/posts/seven/711.png",
        link: "https://sites.google.com/view/rakutenchiase/b%C3%A0i-vi%E1%BA%BFt-h%C6%B0%E1%BB%9Bng-d%E1%BA%ABn/711/sevenbank"
    },
    {
        title: "Cách làm món Tonkatsu chuẩn vị Nhật",
        summary: "Hướng dẫn từng bước để làm món thịt heo chiên xù Tonkatsu giòn rụm, thơm ngon ngay tại nhà.",
        imageUrl: "https://placehold.co/400x250/FBD38D/ffffff?text=Tonkatsu",
        link: "#post-2"
    },
    {
        title: "Học từ vựng tiếng Nhật qua bài hát",
        summary: "Phương pháp học từ vựng hiệu quả và thú vị thông qua các bài hát J-Pop nổi tiếng.",
        imageUrl: "https://placehold.co/400x250/ECC94B/ffffff?text=Học+Tiếng+Nhật",
        link: "#post-3"
    },
];



// --- DỮ LIỆU CHO BÀI VIẾT HƯỚNG DẪN ---
const guidePosts = [
    {
        title: "Cách đăng ký tài khoản Rakuten",
        summary: "Hướng dẫn chi tiết từng bước để tạo tài khoản mua sắm.",
        imageUrl: "https://placehold.co/120x80/FBF7F0/718096?text=HD+1",
        link: "#guide-1"
    },
    {
        title: "Cách cài cấu hình APN sim data softbank, docomo...",
        summary: "Hướng dẫn tải và cài cấu hình sim data các nhà mạng như softbank và docomo.....",
        imageUrl: "img/posts/APN img.png",
        link: "https://sites.google.com/view/rakutenchiase/b%C3%A0i-vi%E1%BA%BFt-h%C6%B0%E1%BB%9Bng-d%E1%BA%ABn/khac/apn"
    }
];

// --- DỮ LIỆU CHO VIDEO HƯỚNG DẪN ---
const videoGuides = [
    {
        title: "Video HD nạp tiền vào thẻ Line Pay",
        summary: "Mô tả ngắn về nội dung video. Thời lượng: 5:30.",
        imageUrl: "https://placehold.co/120x80/FBF7F0/718096?text=Video+1",
        link: "#video-1"
    },
    {
        title: "Video HD cách đi tàu điện ngầm ở Tokyo",
        summary: "Mô tả ngắn về nội dung video. Thời lượng: 8:15.",
        imageUrl: "https://placehold.co/120x80/FBF7F0/718096?text=Video+2",
        link: "#video-2"
    }
];