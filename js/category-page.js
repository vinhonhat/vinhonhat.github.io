document.addEventListener('DOMContentLoaded', function() {
    ...
    // Đồng hồ và ngày tháng (giữ lại từ trang chủ)
    ...
    function loadContentByCategory(postTypes, category, containerId) {
        ...
        // BƯỚC 4: Tạo HTML và chèn vào trang (ĐÃ CẬP NHẬT)
        let contentHtml = '';
        for (const item of latestContent) {
            
            // ================== LOGIC SỬA LỖI ẢNH ==================
            let imageUrl;
            // Nếu đường dẫn ảnh bắt đầu bằng 'http', dùng nó trực tiếp.
            if (item.imageUrl.startsWith('http')) {
                imageUrl = item.imageUrl;
            } else {
                // Nếu không, đó là ảnh nội bộ, thêm '../../' vào.
                imageUrl = `../../${item.imageUrl}`;
            }
            // ========================================================

            // ================== BỐ CỤC HTML MỚI GIỐNG TRANG CHỦ ==================
            // Sử dụng flexbox để chia cột: ảnh trái, chữ phải.
            // Bố cục này tự động co lại đẹp trên điện thoại.
            contentHtml += `
                <a href="${item.link}" target="_blank" class="flex items-start space-x-4 group p-4 bg-white rounded-lg shadow-md transition-shadow duration-300 hover:shadow-lg">
                    <img src="${imageUrl}" alt="${item.title}" class="w-24 h-24 md:w-32 md:h-24 rounded-lg object-cover transition-transform duration-300 group-hover:scale-105 flex-shrink-0">
                    <div class="flex-1">
