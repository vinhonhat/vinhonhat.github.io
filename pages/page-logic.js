...
function initializePage() {
    ...
    const renderMainContent = () => {
        ...
        postsToShow.forEach(post => {
            const imageUrl = post.imageUrl || 'https://placehold.co/400x250/ccc/ffffff?text=No+Image';
    ...
    const renderSuggestions = () => {
        ...
        suggestions.forEach(post => {
            const imageUrl = post.imageUrl || 'https://placehold.co/64x64/ccc/ffffff?text=...';
            elements.suggestionsContainer.innerHTML += `
                <a href="${post.link || '#'}" class="flex items-center p-2 rounded-lg hover:bg-yellow-100 transition-colors duration-200 group mb-4">
